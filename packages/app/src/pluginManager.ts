...
import { div, hr, toBase64Img } from "@dronecad/element";
...
export class PluginManager implements IPluginManager {
    ...
    private async loadFromRemoteFile(url: string) {
        if (url.endsWith(".dronecadplugin")) {
            ...
            const file = new File([blob], "plugin.dronecadplugin");
    ...
    private validateManifest(manifest: PluginManifest) {
        ...
        if (manifest.engines?.dronecad) {
            const currentVersion = __APP_VERSION__;
            if (!this.satisfiesVersion(currentVersion, manifest.engines.dronecad)) {
                errors.push(
                    `NIELIT Drone CAD version ${currentVersion} does not satisfy ${manifest.engines.dronecad}`,
                );
            }
        }
