...
import { ReferencePlaneNode } from "./model/referencePlaneNode";
...
export class ModelManager extends Observable {
    ...
    initRootNode() {
        const root = new FolderNode({ document: this.document, name: this.document.name });
        const defaultGeom = new FolderNode({ document: this.document, name: "Default geometry" });
        defaultGeom.add(new OriginNode({ document: this.document }));
        defaultGeom.add(new ReferencePlaneNode({ document: this.document, planeType: "Top" }));
        defaultGeom.add(new ReferencePlaneNode({ document: this.document, planeType: "Front" }));
        defaultGeom.add(new ReferencePlaneNode({ document: this.document, planeType: "Right" }));
        root.add(defaultGeom);
        return root;
    }
    ...
    async deserialize(data: { components: Serialized[]; nodes: Serialized[]; materials: Serialized[] }) {
        ...
        const hasDefault = (this.rootNode as FolderNode)
            .children()
            .some((n) => n.name === "Default geometry");
        if (!hasDefault) {
            const defaultGeom = new FolderNode({ document: this.document, name: "Default geometry" });
            defaultGeom.add(new OriginNode({ document: this.document }));
            defaultGeom.add(new ReferencePlaneNode({ document: this.document, planeType: "Top" }));
            defaultGeom.add(new ReferencePlaneNode({ document: this.document, planeType: "Front" }));
            defaultGeom.add(new ReferencePlaneNode({ document: this.document, planeType: "Right" }));
            this.rootNode.insertBefore(this.rootNode.firstChild, defaultGeom);
        }
    }
