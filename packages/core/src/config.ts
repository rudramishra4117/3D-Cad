...
export class Config extends Observable {
    ...
    @serialize()
    get language() {
        const lang = this.getPrivateValue("language", I18n.defaultLanguage());
        return I18n.getLanguages().some((l) => l.language === lang) ? lang : "en";
    }
    ...
    @serialize()
    get navigation3D() {
        const val = this.getPrivateValue("navigation3D", "Solidworks");
        return val === "Blender" || val === "Solidworks" ? val : "Solidworks";
    }
