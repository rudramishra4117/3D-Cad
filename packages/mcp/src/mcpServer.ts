...
export function createServer(): McpServer {
    ...
    const server = new McpServer(
        { name: "dronecad-mcp", version: "0.7.0-beta" },
        {
            instructions:
                "DroneCAD builds editable parametric CAD (a .cd node tree, not a mesh) over a shared OCCT core. Units are millimetres. " +
    ...
    server.registerTool(
        "run_cad_program",
        {
            ...
            description:
                'HEADLESS build (writes .cd/STL files on the server; does NOT change what the user sees in the open browser). If the user wants the model to appear in their open DroneCAD browser tab, use live_run_cad_program instead. Execute a sequence of modeling ops to build a parametric model. Ops run in order; shape inputs reference earlier ops via { ref: "<id>" }. Primitives (box/sphere/cylinder/cone/pyramid), sketches (rect/circle/polygon), and operations (extrude/revolve/boolean) are supported. If documentId is omitted a new document is created. boolean/fillet results are baked (non-parametric); primitives and extrude/revolve stay editable. All lengths/coordinates are millimetres (mm); angles (revolve/rotate/array) are DEGREES. Plan exact dimensions before building, then self-verify with get_properties (compare bounding box against your intended mm) and render_preview before trusting the result.',
            inputSchema: {
    ...
    server.registerTool(
        ...
        async ({ documentId, outPath, binary }) => {
            ...
            const file = outPath ?? tmp(`dronecad-${documentId}.stl`);
    ...
    server.registerTool(
        ...
        async ({ documentId, outPath }) => {
            ...
            const file = outPath ?? tmp(`dronecad-${documentId}.step`);
    ...
    server.registerTool(
        "save_cd",
        {
            ...
            description: "Write the editable .cd document JSON to a file (openable in the DroneCAD app).",
            inputSchema: { documentId: z.string(), outPath: z.string().optional() },
        },
        async ({ documentId, outPath }) => {
            const doc = getDoc(documentId);
            const json = JSON.stringify(serializeDocument(doc));
            const file = outPath ?? tmp(`dronecad-${documentId}.cd`);
    ...
    server.registerTool(
        ...
        async ({ dx, dy, dz, outPath, binary }) => {
            ...
            addBox(doc, { dx, dy, dz });
            const stl = documentToStl(app, doc, { binary: binary ?? true });
            const file = outPath ?? tmp(`dronecad-box-${dx}x${dy}x${dz}.stl`);
            writeFileSync(file, stl);
            return {
                content: [
                    {
    ...
    const bridgeUrl = process.env["DRONECAD_BRIDGE_URL"] ?? DEFAULT_BRIDGE_URL;
    ...
    const bridgeUnavailable = (error: unknown) =>
        textResult(
            JSON.stringify({
                ok: false,
                error: `Live bridge unavailable (${error}). Start it with 'npm run bridge -w @dronecad/mcp' and open the app at http://localhost:8080/?live`,
            }),
        );
    ...
    const liveExportToFile = async (
        ...
        try {
            ...
            const file = outPath ?? tmp(`dronecad-live.${suffix}`);
