// Deep import to avoid the @dronecad/app barrel, which transitively pulls in
// @dronecad/element / @dronecad/ui (web components extending HTMLElement) and would
// fail to evaluate in a pure Node environment. document.ts only depends on
// @dronecad/core, so it is headless-safe.
import { Document } from "@dronecad/app/src/document";
import {
    type IApplication,
    type ICommand,
    type IDataExchange,
...
import { initWasm, ShapeFactory } from "@dronecad/wasm";
...
// core (@dronecad/wasm ShapeFactory) but has no window, no UI, no singleton. This
