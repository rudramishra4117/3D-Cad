...
import type { IApplication, IDocument } from "@dronecad/core";
...
const WASM = readFileSync(path.join(process.cwd(), "packages/wasm/lib/dronecad-wasm.wasm"));
const PORT = 8799;
...
describe("live bridge round-trip (controller <-> bridge <-> browser stand-in)", () => {
    ...
    beforeAll(async () => {
        ...
        attachFakeBrowser(browser, app as unknown as IApplication, document);
