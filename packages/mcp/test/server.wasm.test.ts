...
const WASM_BINARY = readFileSync(path.join(process.cwd(), "packages/wasm/lib/dronecad-wasm.wasm"));
...
describe("MCP server", () => {
    test("an MCP client calls make_box and gets valid STL + an editable .cd", async () => {
        ...
        const outPath = path.join(os.tmpdir(), "dronecad-mcp-make-box.stl");
    ...
    test("run_cad_program then export_stl across calls (session continuity)", async () => {
        ...
        const outPath = path.join(os.tmpdir(), "dronecad-prog.stl");
    ...
    test("closed loop: run_cad_program -> render_preview -> export_stl", async () => {
        ...
        // AI exports
        const outPath = path.join(os.tmpdir(), "dronecad-loop.stl");
