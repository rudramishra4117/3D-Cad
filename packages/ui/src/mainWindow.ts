    ...
    protected async fetchIconFont() {
        try {
            ...
            if (response.ok) {
                ...
                console.warn(`Failed to fetch icon font: ${response.statusText}`);
            }
        } catch (e) {
            console.warn(`Failed to fetch icon font: ${e}`);
        }
    }
...
customElements.define("dronecad-main-window", MainWindow);
